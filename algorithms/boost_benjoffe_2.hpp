...
#ifndef EAF_OTHERS_BOOST_BENJOFFE_2_HPP
#define EAF_OTHERS_BOOST_BENJOFFE_2_HPP

#include "eaf/date.hpp"

#include <cstdint>

struct boost_benjoffe_2 {
  ...
  date32_t to_date(int32_t dayNumber) {
    ...
    // the next 6 lines are all now different from boost
    uint32_t a = (dayNumber + 32044 + ajustment);
    uint32_t a2 = a * 4 + 3;
    uint32_t b = a2/146097;
    uint32_t c = a2 + (4*b - (b & ~ 3));
    uint32_t d = c/1461;
...
#endif // EAF_OTHERS_BOOST_BENJOFFE_2_HPP
