...
/**
 * @file neri_schneider_eras.hpp
 *
 * @brief Algorithms on the Gregorian calendar.
 *
 * This code is a supplementary material to [1]:
 *
 ...
 * 
 *     [2] Ben Joffe - "A New Technique for Overflow-Safe Fast Date Algorithms"
 *     https://www.benjoffe.com/safe-date
 */

#ifndef EAF_ALGORITHMS_NERI_SCHNEIDER_ERAS_H
#define EAF_ALGORITHMS_NERI_SCHNEIDER_ERAS_H

#include "eaf/date.hpp"

#include <cstdint>

struct neri_schneider_eras {

  // Shift and correction constants.
  // Note: s changed from 82 to bigger number for
  // overflow safe variant
  static uint32_t constexpr s = 14694;
   ...
   * 
   * General algorithm from [1].
   * Modified as supplementary material to [2]:
   *
   * Modified for blog post:
   * 
   *     [1] Neri C, and Schneider L, "Euclidean Affine Functions and their
   *     Application to Calendar Algorithms" (2022).
   * 
  ...
  date32_t to_date(int32_t N_U) {
    ...
    // Overflow safe variant.
    // Get eras before Century.
    uint32_t const eras = N / 146097;
    uint32_t const doe = N % 146097;

    // Century.
    uint32_t const N_1 = 4 * doe + 3;
    ...
    int32_t  const Y_G = Y + eras * 400 - L + J;
  ...
  int32_t to_rata_die(int32_t Y_G, uint32_t M_G, uint32_t D_G) {
    ...
    // Rata die.
    uint32_t const y_star = 365 * Y + Y / 4 - C + C / 4;
...
#endif // EAF_ALGORITHMS_NERI_SCHNEIDER_ERAS_H
